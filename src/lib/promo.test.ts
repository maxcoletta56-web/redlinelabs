...
test("takes 10 percent off catalogue orders of $200 or more", () => {
  ...
  const priced = priceOrderCents(20000, null);
  assert.equal(priced.discountedCents, 18000);
  const withPromo = priceOrderCents(20000, lookupPromo("DGC20"));
  assert.equal(withPromo.volumeDiscountCents, 2000);
  assert.equal(withPromo.promoDiscountCents, 3600);
  assert.equal(withPromo.discountedCents, 14400);
});
