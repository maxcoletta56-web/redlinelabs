...
import { createFileOrderStore, createSqlOrderStore, memoryOrderStore, type OrderSql } from "./order-store.ts";
...
test("persists orders in the sql store and ignores a corrupt payload", async () => {
  ...
  const store = createSqlOrderStore(sql);
  const quote = quoteCart([{ slug: "bacterial-water", option: "10", qty: 1 }], null);
  const order = createPendingOrder({
    id: "rl_sql0000000000000000001",
    quote,
    email: "max@example.com",
    paymentMethod: "card",
  });
  ...
  assert.equal((await store.get(order.id))?.status, "pending");
  assert.equal((await store.get(order.id))?.totalCents, 1000);
  rows.set(order.id, "{");
  assert.equal(await store.get(order.id), null);
});
