...
> **Canonical path:** `reports/catalogue-merchandising-health.md`. Do not keep a second full copy at the repo root.
...
- Page-render failures and which deployment the public domain serves → `website-health.md`
...
- [x] All `featuredSlugs` / `premiumSlugs` resolve to existing products. Premium slugs are not rendered (`premiumProducts()` has no caller).
- [ ] Every product has at least one variant and consistent `minPrice` / `maxPrice`. Eleven listings have `variants: []`. Where variants exist, min/max match.
- [x] No duplicate `slug` or `sku` values (parent or variant).
- [ ] Every product `image` loads from the allow-listed remote host. 16/30 return PNG; 14/30 return HTTP 403.
- [x] Each product's `categories` are members of the canonical `categories` list.
- [ ] Search matches by name, sku, slug, description, and category. Variant SKUs and option text are not included.
- [x] Related products return same-category items only. Three singleton categories get an empty row.

## Current status

Audited 2026-09-25 against `src/data/products.json` (30 listings) and the home page, shop, product page, and related-product row. Identity keys are clean. The catalogue is not presentation-complete: blocked images (including featured Tesamorelin), listings with no dose option, parent SKU shown instead of the selected option, unused premium picks, and thin related rows.

The public site was still on an older deployment at audit time, so these findings describe the repository catalogue, not the HTML currently on https://redlinelabs.shop. See `website-health.md`.
...
Fourteen image URLs fail at the allowed image host, including the featured Tesamorelin card and three of the four premium slugs. Eleven listings publish a price and a parent SKU but an empty `variants` array, so the shop and product page cannot show a dose. The product page always prints the parent SKU, which often is not the SKU of the option on screen. Premium picks are defined and never rendered. Three listings have no related products because they are alone in their category.
...
| Date | Agent | Summary |
| --- | --- | --- |
| 2026-09-25 | Catalogue & Merchandising | Full catalogue audit of 30 listings (images, variants, SKU, merchandising). |
| 2026-09-25 | Website health coordination | Moved that audit into this canonical file. Root copy replaced with a pointer. |
